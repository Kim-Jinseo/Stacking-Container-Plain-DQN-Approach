...
class DuelingNstepDQN(nn.Module):
    def __init__(self, width, height, action_num):
        ...
        self.fc = nn.Linear(width * height * 64, 128)
...
# Models on device
# -----------------------
dqn = DuelingNstepDQN(width, height, action_number).to(device)
ema_dqn = DuelingNstepDQN(width, height, action_number).to(device)
ema_dqn.load_state_dict(dqn.state_dict())
ema_dqn.eval()

target_dqn = DuelingNstepDQN(width, height, action_number).to(device)
...
def simulate_action(grid, action_index):
    ...
    reward += 0.06 * (phi_after - phi_before)
...
def train_from_replay():
    ...
    states, actions_batch, returns, next_states, dones = zip(*batch)
    ...
    q_pred = dqn(states).gather(1, actions_batch.unsqueeze(1)).squeeze(1)
    targets = returns
    ...
    for ema_param, param in zip(ema_dqn.parameters(), dqn.parameters()):
        ema_param.data.mul_(1 - tau).add_(tau * param.data)
...
for step in range(max_depth):
    ...
    for g, total_r, traj, done, visited in beam:
        with torch.no_grad():
            qvals = ema_dqn(grid_to_tensor(g))
        ...
        for a in topk.tolist():
            ...
            candidates.append((ng, total_r + r, traj + [(g, a, r)], d, new_visited))
